...
The `home-routes` handler in the `graphql-toy.routes.home` namespace
...
The `home-page` function will in turn call the `graphql-toy.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `graphql-toy.handler` namespace:
...
Request middleware functions are located under the `graphql-toy.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `graphql-toy.dev-middleware` namespace found in
